...
def split_augmented_samples(data, train_percentage=.6, validation_percentage=.3, test_percentage=.1,
    ...
    print(f"{num_of_diff_samples} unique patients' samples have been duplicated, by a factor of {duplicate_factor}")
    ...
    print(f"{trains_added} unique patients' samples to add to training dataset")
    print(f"{vals_added} unique patients' samples to add to validation dataset")
    print(f"{tests_added} unique patients' samples to add to testing dataset")
    ...
    to_val = duplicates.iloc[
             trains_added * duplicate_factor:trains_added * duplicate_factor + vals_added * duplicate_factor, :]
    to_test = duplicates.iloc[trains_added * duplicate_factor + vals_added * duplicate_factor:, :]

    if to_train.shape[0] % duplicate_factor != 0 or to_val.shape[0] % duplicate_factor != 0 or to_test.shape[
        0] % duplicate_factor != 0:
        raise ValueError
